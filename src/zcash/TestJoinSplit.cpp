...
bool test_prove(
        ZCJoinSplit* js,
        const std::array<JSInput, ZC_NUM_JS_INPUTS>& inputs,
        const std::array<JSOutput, ZC_NUM_JS_OUTPUTS>& outputs,
        std::array<Note, ZC_NUM_JS_OUTPUTS>& out_notes,
        std::array<ZCNoteEncryption::Ciphertext, ZC_NUM_JS_OUTPUTS>& out_ciphertexts,
        uint256& out_ephemeralKey,
        const uint256& pubKeyHash,
        uint256& out_randomSeed,
        std::array<uint256, ZC_NUM_JS_INPUTS>& out_hmacs,
        std::array<uint256, ZC_NUM_JS_INPUTS>& out_nullifiers,
        std::array<uint256, ZC_NUM_JS_OUTPUTS>& out_commitments,
        uint64_t vpub_old,
        uint64_t vpub_new,
        const uint256& rt,
        const uint256& id,
...
bool test_multi_asset_joinsplit(ZCJoinSplit* js, std::map<uint256, MultiAssetAccount>& maas, ZCIncrementalMerkleTree& tree, std::set<uint256>& nullifiers_set) {
    ...
    std::array<uint256, maas_len / 2> pubKeyHash;
    // 创建随机种子, 用于生成 JoinSplit 的签名
    std::array<uint256, maas_len / 2> randomSeed;
    std::array<std::array<uint256, 2>, maas_len / 2> macs;
    std::array<std::array<uint256, 2>, maas_len / 2> nullifiers;
    std::array<std::array<uint256, 2>, maas_len / 2> commitments;

    std::array<uint64_t, maas_len / 2> vpub_old;
    std::array<uint64_t, maas_len / 2> vpub_new;
    uint256 rt;
    ...
    for (int i = 0; i < maas_len / 2; ++i) {
        ...
        ZCProof proof;
        //struct timeval start, end;
        inputs[i][0] = JSInput(id1);
        inputs[i][1] = JSInput(id1);
        
        outputs[i][0] = JSOutput(recipient_addr1, v1, id1);
        outputs[i][1] = JSOutput(recipient_addr2, v2, id1);
        
        //gettimeofday(&start, NULL);
        ths.push_back(thread(&test_prove, js, 
                            ref(inputs[i]), 
                            ref(outputs[i]),
                            ref(out_notes[i]),
                            ref(ciphertexts[i]),
